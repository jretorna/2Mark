...
<h2><?php
echo h($currentStudent['lastname']) . " " . h($currentStudent['firstname']); ?></h2>
...
if(!empty($marks)){?>
    ...
    <?php echo $this->Html->link('Ajouter une nouvelle note', array('controller' => 'marks', 'action' => 'add', $currentStudent['id']), array('class' => 'btn btn-success')); ?>
    <table>
        <tr>
            <th>Id</th>
            <th>Matière</th>
...
<?php }else{?>
  ...
  Cet(te) étudiant(e) n'a aucune note. <?php echo $this->Html->link('Ajouter en une !', array('controller' => 'marks','action' => 'add', $currentStudent['id'])); ?>
</div>
<?php
}
